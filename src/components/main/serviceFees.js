...
class ServiceFeesComponent extends React.Component {
	constructor(props) {
        super(props);
        this.state = {
            currentPage: 1,
...
// 获取列表数据
    getList = () => {
        ...
        actions.getServiceFees(this.param, (json) => {
...
// 分页事件
   ...
   // 删除二维码管理
    ...
    del = (id) => {
        const { actions } = this.props;
        let param = {ids: id};
          actions.delServiceFees(param, (json) => {
            if(json.code==200){
    ...
    jumpToTabPage = () => {
        ...
        //     localStorage.removeItem("code");
            browserHistory.push(`/main/settings/serviceFees_add`);
        // } else {
 ...
 const columns  = [
            ...
            {
                title: '状态',
                dataIndex: 'tsf_status',
                key: 'tsf_status',
                sorter: true,
                render:val => <span>{val==1?'正常':'停用'}</span>
            },
            {
                title: '操作',
                dataIndex: '4',
                    ...
                    return (
                        <div className="table-icon-group">
                            <Icon  className="icon-right"
                                type="delete"
                                onClick={() => {
                                    this.showConfirm(record.tsf_id, record.tsf_name)
                                }}
                            />
                        </div>
                    )
                        ...
                        <Link to="/main/settings/code" >二维码管理</Link>
                         ...
                         <Link to="/main/settings/item" >故障类型</Link>
                          <Link to="/main/settings/brand" >品牌管理</Link>
                           <Link to="/main/settings/serviceFees" activeClassName="active">售后套餐</Link>
                    </div>
...
export default connect(mapStateToProps, buildActionDispatcher)(ServiceFeesComponent);
