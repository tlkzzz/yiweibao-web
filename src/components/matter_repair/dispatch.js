  ...
  workOrder_state=(e)=>{
    ...
    render() {
      ...
      let workOrderIdSign=localStorage.getItem('workOrderIdSign');
        ...
        let operationAuthority=null;//权限人员id
         this.state.operationAuthority=null;//清空历史值
         if(dispatchWorkOrderInfo&&dispatchWorkOrderInfo.workOrderStatus=='DYS'){
            description=dispatchWorkOrderInfo.acceptDescription
          }
        let processOptionExplain=[];
          if(dispatchWorkOrderInfo&&dispatchWorkOrderInfo.workOrderStatus=="DTB"){
              processOptionExplain[0]=dispatchWorkOrderInfo?dispatchWorkOrderInfo.reportPersonName:null;
              //创建人
              //operationAuthority=
          }
          else if(dispatchWorkOrderInfo&&dispatchWorkOrderInfo.workOrderStatus=="DFP"){
              processOptionExplain[0]=dispatchWorkOrderInfo?dispatchWorkOrderInfo.executionPerson:null;
              //提报人
              this.state.operationAuthority=dispatchWorkOrderInfo.reportPersonId
          }else if(dispatchWorkOrderInfo&&dispatchWorkOrderInfo.workOrderStatus=="DHB"){
              processOptionExplain[0]=dispatchWorkOrderInfo?dispatchWorkOrderInfo.dispatchPersonName:null;
              //执行人
              this.state.operationAuthority=dispatchWorkOrderInfo.executionPersonId
          } else if(dispatchWorkOrderInfo&&dispatchWorkOrderInfo.workOrderStatus=="DYS"){
              //分派人
              processOptionExplain[0]=dispatchWorkOrderInfo?dispatchWorkOrderInfo.dispatchPersonName:null;
              processOptionExplain[1]=dispatchWorkOrderInfo?dispatchWorkOrderInfo.executionPerson:null;
              this.state.operationAuthority=dispatchWorkOrderInfo.dispatchPersonId
          }
                ...
                <div>
                    <div className="top-bar clearfix">
                        <div className="details-title pull-left">
                            <h3>{entity ? entity.workOrderNum : code }</h3>
                            <span className="eam-tag">{entity ? entity.workOrderStatusDescription : '待提报' }</span>
                            <p>{entity ? entity.reportRemarks : '' }</p>

                        </div>
                        <div className="list-tools-right pull-right">
                            {
                                !dispatchIsAdd? <DetailsPagination
                                        state={state} // 此模块state
                                        listDataName="dispatchWorkOrderListData" // 列表数据state名 -> data = state.workOrderListData
                                        localStorageName="dispatchWorkOrder" // onChang 方法内设置的存储名
                                        onChange={(record)=>{
                                            let json = {};
                                            json.id = record.workOrderId; // *跳转前存相关数据 和列表页跳详情页做同样处理 (这个存储是必要的操作并且必须包含id)
                                            let status = this.dispatchOrderConfigJson[record.workOrderStatus];
                                    ...
                                    <Dropdown
                                        overlay={(
                                            <Menu onClick={(e) => {this.detailsMoreClick(e.key)}}>
                                                {
                                                    entity && entity.workOrderStatus!= 'QX'&&entity.workOrderStatus!= 'GB' ?
                                                        <Menu.Item key="0"><Icon type="edit" /> 变更状态</Menu.Item> :
                                                        null
                                                }
                                                {
                                                    entity && entity.workOrderStatus === 'DTB' ?
                        ...
                        <div className="eam-tab-nav">
                            <Link activeClassName="active" to="/matter_repair/dispatch/dispatch_tab1"><Icon type="check-circle-o"/> 工单提报</Link>
                            <Link activeClassName="active" to="/matter_repair/dispatch/dispatch_tab2"><Icon type="check-circle-o"/> 任务分派</Link>
                            <Link activeClassName="active" to="/matter_repair/dispatch/dispatch_tab3"><Icon type="check-circle-o"/> 执行汇报</Link>
                            <Link activeClassName="active" to="/matter_repair/dispatch/dispatch_tab4"><Icon type="check-circle-o"/> 验收确认</Link>
                    ...
                    <SendProcess
                        key="9"
                        status={entity?entity.workOrderStatus:false}
                        ...
                        data={businessJson?businessJson.dispatchOrder?businessJson.dispatchOrder.dispatchOrderStatus:null:null}
                    />
                </div>
            ) :
            (
