...
class BacklogMaintenanceWorkOrderComponent extends React.Component {
    constructor(props) {
        ...
        PubSub.subscribe(pubTopic.task.TO_TASK.PATROLORDER, this.queryKeyword );
    ...
    myTaskRefresh=(msg,data)=>{
        ...
        if(data=="patrolOrder"){
            this.queryAction.myTaskBacklogGetPatrolList(this.param);
        }
    }
    //跳转
    jumpToTabPage = (record) => {
            ...
            switch(record.orderType)
        {
            case "patrolOrder"://巡检工单
                ...
                browserHistory.push(`/patrol/order/${status.path}?from=/task/backlog/backlog_tab5`);
                break;
            default:
                message.error("数据异常");
        }
...
// 表格事件-排序
    ...
    componentWillMount () {
        ...
        actions.myTaskBacklogGetPatrolList(this.param);
    }
    //查询工单状态过滤
    queryStatus=(status)=>{
        if(status){
...
class BacklogFiveComponent extends React.Component {
 constructor(props) {
        super(props);
        // 高级筛选选项数据
...
export default connect(mapStateToProps, buildActionDispatcher)(BacklogFiveComponent);