...
class MaintenanceWorkOrderComponent extends React.Component {
    constructor(props) {
        ...
        PubSub.subscribe(pubTopic.task.HANDILING_TASK.PATROLORDER, this.getPush );
        //表格多选
        this.rowSelection = {
        ...
        try {
            switch(record.orderType)
            {
                case "patrolOrder"://巡检工单
                    ...
                    browserHistory.push(`/patrol/order/${status.path}?from=/task/handle/handle_tab5`);
                    break;
                default:
                    message.error("数据异常");
            }
...
// 表格事件-排序
    tableChange = (pagination, filters, sorter) => {
        ...
        this.queryAction.myTaskHandleGetPatrolList(this.param);
    }
    // 分页事件
    ...
    componentWillMount () {
       ...
       //// console.log("----------componentWillMount----commonState------",{orderStatus:'待分派',sorts:'desc'},commonState);
        actions.myTaskHandleGetPatrolList(this.param);
        this.getStatusData();
...
class HandleFiveComponent extends React.Component {
	constructor(props) {
        super(props);
        // 高级筛选选项数据
...
export default connect(mapStateToProps, buildActionDispatcher)(HandleFiveComponent);