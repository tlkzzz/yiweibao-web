...
class ServicePointsComponent extends React.Component {
	constructor(props) {
        super(props);
        this.state = {
            currentPage: 1,
...
// 获取列表数据
    getList = () => {
        ...
        actions.getServicePoints(this.param, (json) => {
...
// 分页事件
   ...
   // 删除管理
    ...
    del = (id) => {
        const { actions } = this.props;
        let param = {ids: id};
          actions.delServicePoints(param, (json) => {
            if(json.code==200){
    ...
    jumpToTabPage = () => {
        ...
        //     localStorage.removeItem("code");
            browserHistory.push(`/main/settings/servicePoints_add`);
        // } else {
 ...
 const columns  = [
            ...
            {
                title: '联系人',
                dataIndex: 'tsp_charge_person',
                key: 'tsp_charge_person',
                sorter: true,
            },
            {
                title: '电话',
                dataIndex: 'tsp_charge_phone',
                key: 'tsp_charge_phone',
                sorter: true,
            },{
                title: '地址',
                ...
                title: '创建时间',
                dataIndex: 'tspAddDate',
                key: 'tspAddDate',
                sorter: true,
            },
            {
                title: '状态',
                dataIndex: 'tsp_status',
                key: 'tsp_status',
                sorter: true,
                render:val => <span>{this.points_state(val)}</span>
            },
            {
                title: '操作',
                dataIndex: '4',
                    ...
                    return (
                        <div className="table-icon-group">
                            <Icon  className="icon-right"
                                type="delete"
                                onClick={() => {
                                    this.showConfirm(record.tsp_id, record.tsp_name)
                                }}
                            />
                        </div>
                    )
        ...
        return (
             ...
             <div className="main-nav clearfix">
                        ...
                        <Link to="/main/settings/servicePoints" activeClassName="active">项目管理</Link>
...
export default connect(mapStateToProps, buildActionDispatcher)(ServicePointsComponent);
